...
def plot_moment_matching(barphi_e, barphi_p, t, threshold):
    ...
    plt.plot(t[2:],((barphi_e[:,idx_non_negligeable]-barphi_p[:,idx_non_negligeable]).abs()/(barphi_e.abs()[:,idx_non_negligeable]+barphi_p.abs()[:,idx_non_negligeable])).sum(1)[2:-1].cpu(),marker='.')
    plt.xlabel('t')
    plt.yscale('log')
